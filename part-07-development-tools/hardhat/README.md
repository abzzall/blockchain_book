...
- **`0x80ac58cd` is derived, not magic.** Combining the nine ERC-721 function
  ...
  `part-03-ethereum/ethereum/contracts.py` computes the selectors with the
...
`part-03-ethereum/ethereum/` derives from the published test mnemonic and the
...
`create_address` in `part-03-ethereum/ethereum/` computes for the same inputs.
