...
Verified on 2026-09-06 with **Foundry 1.8.1** and **solc 0.8.36**, both pinned
in `foundry.toml`. All **52 tests pass**. No network, no node, no wallet, no
...
- **`Constants.sol`** — `constant` is fixed when the source is compiled,
  `immutable` when the contract is deployed, and neither occupies storage.
  `forge inspect ... storageLayout` shows the point plainly: the contract
  declares five state variables and uses **one** slot.
...
same values `samples-and-code/ethereum/contracts.py` computes from the
